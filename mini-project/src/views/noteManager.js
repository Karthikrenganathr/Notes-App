...
export default class noteManagerView{
    expandTitle(){
        const contentDiv=document.querySelector(".notes__creatediv__content .notes__creatediv__content__enter");
        contentDiv.addEventListener("focus", () => {
            const titleDiv=document.querySelector(".notes__creatediv__titlediv");
            const optionsDiv=document.querySelector(".notes__creatediv__createOptionscon");
            titleDiv.style.display = "block";
            optionsDiv.style.display = "block";
        });
    }
    headerPinUnpin() {
        document.querySelector('.notes__creatediv__titlediv__pinContainer').addEventListener('click', (event) => {
    ...
    createNoteElement(title, description, id) {
        ...
        <img src="./img/note/more.svg" alt="alert image" class="note__options__img">
                ...
                `;
        noteDiv.querySelector(".note__mainnote").addEventListener("click",()=>this.enlargeView(id));
    ...
    createNote(title,description,result,isPinned){
        document.querySelector(".notes__creatediv__content .notes__creatediv__content__enter").value="";
        document.querySelector(".notes__creatediv__titlediv .notes__creatediv__content__enter").value="";
        const titleDiv=document.querySelector(".notes__creatediv__titlediv");
        const optionsDiv=document.querySelector(".notes__creatediv__createOptionscon");
        if(window.createStartPin){
            const imgElement = document.querySelector(".enlarged__note__imgcontainer__img");
            imgElement.setAttribute("src", "../../img/note/unpin.svg"); 
            imgElement.setAttribute("alt", "header pin");
        ...
        if(isPinned)
        {
            let img=document.querySelector('.notes__creatediv__titlediv__pinContainer img');
            ...
            noteElement.querySelector(".note__imgcontainer__img").addEventListener("click",noteController.unPinNote.bind(null,result.id));
            window.pinnedCount++;
        }
        else{
            const notesContainer = document.querySelector(".notes__unpin__noteCon");
            notesContainer.prepend(noteElement);
            noteElement.querySelector(".note__imgcontainer__img").addEventListener("click",noteController.pinNote.bind(null,result.id));
        }
        noteElement.querySelector(".note__options img:nth-child(5)").addEventListener("click",  noteController.archiveNote.bind(null,result.id));
    }
    viewNote(notes){
        notes.notes.forEach((note)=> {
            const noteElement = this.createNoteElement(note.title, note.text,note.id);
            if(note.isPinned)
            {
                if(window.pinnedCount==0)
                {
                ...
                noteElement.querySelector(".note__imgcontainer__img")?.setAttribute("src", "./img/note/unpin.svg");
                noteElement.querySelector(".note__imgcontainer__img")?.setAttribute("alt", "pin image");
                const notesContainer = document.querySelector(".notes__pin__noteCon");
                notesContainer.prepend(noteElement);
                noteElement.querySelector(".note__imgcontainer__img").addEventListener("click",noteController.unPinNote.bind(null,note.id));
                window.pinnedCount++;
            }
            else{
                const notesContainer = document.querySelector(".notes__unpin__noteCon");
                notesContainer.prepend(noteElement);
                noteElement.querySelector(".note__imgcontainer__img").addEventListener("click",noteController.pinNote.bind(null,note.id));
            }
            noteElement.querySelector(".note__options img:nth-child(5)").addEventListener("click",  noteController.archiveNote.bind(null,note.id));
        });
    }
    enlargeView(id) {
        let noteElement = document.querySelector(`[noteId='${id}']`);
        let hiddenDiv = document.querySelector(".enlarged");
        hiddenDiv.querySelector(".enlarged__note__mainnote").textContent =noteElement.querySelector(".note__mainnote").textContent;
        hiddenDiv.querySelector(".enlarged__note__heading").textContent =noteElement.querySelector(".note__heading").textContent;
