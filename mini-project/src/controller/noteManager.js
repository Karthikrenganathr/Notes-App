...
export default class noteManagerController{
    ...
    saveNote(){
        document.querySelector(".notes__creatediv__createOptionscon__closecon").addEventListener("click",async()=>{
            let content=document.querySelector(".notes__creatediv__content .notes__creatediv__content__enter").value;
            let title=document.querySelector(".notes__creatediv__titlediv .notes__creatediv__content__enter").value;
    ...
    closeNote(){
        document.querySelector('.enlarged__note__options__close').addEventListener("click",async()=>{
            let englargedNote = document.querySelector(".enlarged");
            let title=englargedNote.querySelector(".enlarged__note__heading").textContent;
            let content=englargedNote.querySelector(".enlarged__note__mainnote").textContent;
            ...
            selectedNote.querySelector(".note__mainnote").textContent = englargedNote.querySelector(".enlarged__note__mainnote").textContent;
            selectedNote.querySelector(".note__heading").textContent = englargedNote.querySelector(".enlarged__note__heading").textContent;
    ...
    async MainPage(){
        const mainClick = document.querySelector(".sidesection__container:nth-child(1)"); 
        mainClick.addEventListener("click", async function () {
            if(window.currentPage!=="mainPage")
            {
                ...
                const archive=document.querySelector(".sidesection__container:nth-child(2)")
    ...
    search() {
        ...
        function getNotes() {
            return Array.from(document.querySelectorAll(".note")).map(note => ({
                ...
                titleWords: note.querySelector(".note__heading").textContent.toLowerCase().split(" "),
                textWords: note.querySelector(".note__mainnote").textContent.toLowerCase().split(" ")
            }));
        }
        ...
        function searchSingleNote(note, searchWords) {
            return new Promise(resolve => {
                ...
                if (searchWords.some(word => cachedResult.falseSearchWords.includes(word))) {
                    resolve({
                        noteId: note.id,
                ...
                let newTrueSearchWords = [], newFalseSearchWords = [];
                let wordsToCheck = searchWords.filter(word => 
                ...
                let finalTrueSearchWords = [
                    ...cachedResult.trueSearchWords.filter(word => searchWords.includes(word)), 
                    ...newTrueSearchWords
                ];
                let finalFalseSearchWords = [
                    ...cachedResult.falseSearchWords.filter(word => searchWords.includes(word)), 
                    ...newFalseSearchWords
                ];
        ...
        async function handleSearch(event) {
            ...
            if (!Array.isArray(window.searchArray) || window.searchArray.length === 0) {
                window.searchArray =getNotes();
                console.log(window.searchArray);
            }
    
            let results = await searchNotesParallel(window.searchArray, searchText.split(" "));
            console.log("Search Results:", results);
            results.forEach(result => {
                let noteElement = document.querySelector(`.note[noteid='${result.noteId}']`);
                if (noteElement) {
        ...
        const searchInput = document.querySelector(".header__searchContainer__search");
